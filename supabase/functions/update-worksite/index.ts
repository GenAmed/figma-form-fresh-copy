...
serve(async (req) => {
  ...
  try {
    ...
    // Récupérer les données du chantier à mettre à jour
    const worksiteData = await req.json();
    const { id, ...updateData } = worksiteData;
    
    // Log pour debugging
    console.log("Mise à jour du chantier:", id);
    console.log("Données:", JSON.stringify(updateData));

    // Mettre à jour le chantier dans la table worksites
    ...
    console.log("Chantier mis à jour avec succès:", data);
    return new Response(
      JSON.stringify({ data }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
